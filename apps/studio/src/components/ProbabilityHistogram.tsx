...
export default function ProbabilityHistogram({ probabilities, numQubits }: ProbabilityHistogramProps) {
  ...
  return (
    <div className="overflow-x-auto pb-2">
      <div className="flex items-end gap-1 h-28 min-w-max">
        {probabilities.map((prob, idx) => {
          const heightPercent = (prob / maxProb) * 100;
          const binaryLabel = idx.toString(2).padStart(numQubits, '0');
          return (
            <div key={idx} className="flex flex-col items-center w-10 min-w-[24px]" data-testid="probability-bar">
              <span className="text-xs text-green-400 mb-1 font-mono">
                {(prob * 100).toFixed(1)}%
              </span>
              <div
                className="w-full bg-gradient-to-t from-anvaya-cobalt to-anvaya-light rounded-t-sm transition-all duration-300"
